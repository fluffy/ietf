...
This specification defines a new SDP framework for identifying Source RTP Streams (Section 2.1.10 {{I-D.ietf-avtext-rtp-grouping-taxonomy}}) called
" RTP Payload Format Constraints Identifier" with the constraints on its payload format in the Session Description Protocol. The "rid" framework can be thought as complementary extension to the way the media format parameters are specified in SDP today, via the "a=fmtp" attribute. This specification also proposes a new RTP header extension to carry the "rid" value for provding the correlation between the RTP Packets to their format specification in the SDP.
...
an unique RTP payload format identified via the "rid-identifier".
...
({{sec-rid_level_attributes}}) further constrain the equivalent 'fmtp' attributes. 
...
Following new SDP parameters shall be defined that represent things common across video codecs.
* max-width, for spatial resolution in pixels
* max-height, for spatial resolution in pixels
* max-fps, for frame rate in frames per second
* max-fs, for frame size in pixels per frame
* max-br, for bit rate in bits per second
* max-pps, for pixel rate in pixels per second
...
include one or more "a=rid" lines to constrain the payload format 
specification for the given set of RTP Payload Types. 
